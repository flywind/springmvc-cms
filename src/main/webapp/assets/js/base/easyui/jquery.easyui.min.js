...
for(var i=0;i<_84.pageList.length;i++){
	var _thisValue = _84.pageList[i];
	if(_84.pageList[i]=='all' || _84.pageList[i]=='全部'){
		_thisValue = 1000000;
	}
$("<option></option>").text(_84.pageList[i]).attr('value', _thisValue).appendTo(ps);
}
...
td.attr("field",col.field);
if(col.sortable){
	sortStr = "<span class=\"datagrid-sort-icon\"></span>";
}
td.append("<div class=\"datagrid-cell\"><span></span>"+sortStr+"</div>");
...
$('.datagrid-header-check').find('input').attr('checked', false);
...
var rowNumber = (opts.pageNumber-1) * opts.pageSize+i+1;  //重新计算treeGrid的行号，修复分页后行号从1开始的Bug
$(this).html(rowNumber);
});
};
function _795(_796){
...
var emptyView = $.extend({},$.fn.datagrid.defaults.view,{
    onAfterRender:function(target){
        ...
        if (!$(target).datagrid('getRows').length){
            var d = $('<div class="datagrid-empty"></div>').html($.fn.datagrid.defaults.emptyMsg).appendTo(vc);
            d.css({
                position:'absolute',
                left:0,
                top:40,
...
//datagrid数据为空的时候的提示
var emptyTreeView = $.extend({},$.fn.treegrid.defaults.view,{
    onAfterRender:function(target){
        $.fn.datagrid.defaults.view.onAfterRender.call(this,target);
        ...
        if (!$(target).treegrid('getRoots').length){
            var d = $('<div class="datagrid-empty"></div>').html($.fn.treegrid.defaults.emptyMsg).appendTo(vc);
            d.css({
                position:'absolute',
                left:0,
                top:40,
...
window.emptyTreeView = emptyTreeView;   
})(jQuery);
