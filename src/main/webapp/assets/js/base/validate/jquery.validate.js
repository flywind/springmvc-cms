...
(function($) {
    ...
    $.extend($.validator, {
        ...
        messages: {
            ...
            integer: "只能输入整数",
            digital: "只能输入数字和小数点",
            decimalOne: "只能输入数字和最多一位小数",
            decimalTwo: "只能输入数字和最多两位小数",
            decimalFour: "只能输入数字和最多四位小数",
            decimalSix: "只能输入数字和最多六位小数",
            creditcard: "请输入合法的信用卡号",
            equalTo: "请再次输入相同的值",
            notEqualTo: $.validator.format("不能输入与 {0} 相同的值"),
            accept: "请输入拥有合法后缀名的字符串",
            maxlength: $.validator.format("请输入一个长度最多是 {0} 的字符串"),
            maxCharCodelength:$.validator.format("请输入一个长度最多是 {0} 的字符，其中一个汉字占两个字符"),
            ...
            inputLetterOrNumber: "只允许输入字母或数字",
            letterAndNumberAndCharacters: "必须包含字母和数字，支持@、#、$、&、_、.",
            specialCharacters: "不能包含特殊字符",
            firstCharactersChecked: '首字母必须是D,d,M,m，后面输入正整数',
            only: "你输入的值有重复，请重新输入",
            ip: '请输入正确的ip地址',
            port: '请输入正确的端口号'
        },
        ...
        classRuleSettings: {
            ...
            inputLetterOrNumber:{
            	inputLetterOrNumber:true
            },
            letterAndNumberAndCharacters: {
            	letterAndNumberAndCharacters: true
            },
            specialCharacters : {
            	specialCharacters:true
            },
            firstCharactersChecked: {
            	firstCharactersChecked: true
            },
            only:{
                only:true
            },
        ...
        methods: {
            ...
            integer: function(value, element) {
            	return this.optional(element) || /^-?[1-9]\d*$/.test(value) || /^0*$/.test(value);
            },
            //只能输入数字和小数点
            ...
            //只能输入数字和最多一位小数
            decimalOne: function(value, element) {
            	return this.optional(element) || /^(\d+(\.\d{1,2})?)$/g.test(value);
            },
            //只能输入数字和最多两位小数
            decimalTwo: function(value, element) {
            	return this.optional(element) || /^(\d+(\.\d{1,2})?)$/g.test(value);
            },
            //只能输入数字和最多四位小数
            decimalFour: function(value, element) {
            	return this.optional(element) || /^(\d+(\.\d{1,4})?)$/g.test(value);
            },
            //只能输入数字和最多六位小数
            decimalSix: function(value, element) {
            	return this.optional(element) || /^(\d+(\.\d{1,6})?)$/g.test(value);
            },
	      ...
	      //时间小于等于指定时间
            ...
            //字母OR数字
            inputLetterOrNumber: function(value, element){
            	var word = /^[a-zA-Z0-9]{1,}$/;
                return this.optional(element) || (word.test(value));
            },
            
            //字母或数字或@#$&_.
            letterAndNumberAndCharacters: function(value, element){
            	var word = /([a-zA-Z]+[\d]+[@#\$&_\.]*)|([\d]+[a-zA-Z]+[@#\$&_\.]*)|([a-zA-Z]+[@#\$&_\.]*[\d]+)|([\d]+[@#\$&_\.]*[a-zA-Z]+)/;
                return this.optional(element) || (word.test(value));
            },
            
            //特殊字符
            specialCharacters: function(value, element) {
            	var word = /^[a-zA-Z0-9\u4e00-\u9fa5@#\$&_\.]{1,}$/;
                return this.optional(element) || (word.test(value));
            },
            
            //首字母校验
            firstCharactersChecked: function(value, element) {
            	var word = /^[M|m|D|d]+\d+$/;
                return this.optional(element) || (word.test(value));
            },
            ...
            ip: function(value, element) {
            	...
            	var word = /^(\d+)\.(\d+)\.(\d+)\.(\d+)$/g;
            	if (word.test(value) && RegExp.$1 < 256 && RegExp.$2 < 256 && RegExp.$3 < 256 && RegExp.$4 < 256) {
        			return true;
        		}
            	return false;
            },
            ...
            port: function(value, element) {
            	...
            	var word = /^\d+$/;
            	if (word.test(value) && value < 65536) {
        			return true;
        		}
            	return false;
            }
